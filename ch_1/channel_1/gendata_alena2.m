...
for i=1:nx
    for j=1:ny
        if (j<42) 
            icetopo(i,j)= min(Hmax,Hmin + dHdx*(latc(j)-latg(1)));
        else
            icetopo(i,j)=0;
        end
    end
...
%%
 ...
 fid=fopen('icetopo.exp2','w','b'); fwrite(fid,icetopo,acc);fclose(fid);
 fid=fopen('pload.exp2','w','b'); fwrite(fid,-icetopo,acc);fclose(fid);
...
for i=1:nx
    for j=1:ny
        k=max(find(abs(zg)<abs(icetopo(i,j))));
            if isempty(k)
                k=0;
                HFacC(i,j,:)=0;
            end 
            if k>0
                kp1=min(k+1,nz);
                HFacC(i,j,1:(k-1))=0;   
                HFacC(i,j,k)=(abs(zg(kp1))- abs(icetopo(i,j)))/dz(k);
                HFacC(i,j,(k+1):nz)=1;
            end
    end
end
...
fid=fopen(['phi0surf.exp2.' eos],'w','b'); fwrite(fid,phi0surf,acc);fclose(fid);
